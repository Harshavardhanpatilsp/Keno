import { _decorator, Button, color, Component,UIOpacity, EditBox, Label, Node, Sprite, SpriteFrame, tween, Vec3,instantiate,sys,UITransform,randomRangeInt,log, logID, Scene, director } from 'cc';
...
export class AutoPlayManager extends Component {
    ...
    public static stopcashincby: object = {"enable": false, "value": 0.00};
    public static stopcashdecby: object = {"enable": false, "value": 0.00};
    public static stopcashsingleby: object = {"enable": false, "value": 0.00};
    ...
    OneditorboxClicked(event,type){
        ...
        if(type == "EditBox"){
            ...
            if(( AutoPlayManager.curEditbox.name=="winorlose<EditBox>") && Number(AutoPlayManager.curEditbox.string) >100){
                    AutoPlayManager.curEditbox.string = 100.00+""
            }
            if(AutoPlayManager.curEditbox.string == "" || AutoPlayManager.curEditbox.string == "0." || AutoPlayManager.curEditbox.string == "0" || AutoPlayManager.curEditbox.string =="."){
                AutoPlayManager.curEditbox.string = 1.00.toFixed(2);
            }
            let val = this.Numset(AutoPlayManager.curEditbox.node.parent.parent.name);
    ...
    OnRoundNumberClick(event){
        ...
        console.log(AutoPlayManager.Roundsnum,"n times");
    ...
    WinOrLose(event){
        ...
        for(let i =1 ; i < parent.children.length;i++){
            if(parent.children[i] != curnode){
                // curnode.children[3].active=true
    ...
    autogame() {
        // console.log("hi");
        if(this.ballpool.Allbuttons[8].getComponent(Sprite).spriteFrame==this.ballpool.redsprite){
        //   console.log("hello");
          this.Stopautoplay=true
        }
        else{
        
          if(this.ballpool.resultpoints.length>0){
            ...
            for(let i=0;i<this.ballpool.payoutchildren.length;i++){
           ...
           this.ballpool.resultpoints=new Array
          }
        this.ballpool.Allnodes[3].active = true
        }
    ...
    startauto(){
       ...
       this.inside=false
        ...
        this.ballpool.Allbuttons[8].getComponentInChildren(Label).string=autono+""
        this.ballpool.Allbuttons[8].node.getChildByName("Label").active=true
        this.ballpool.Allbuttons[8].getComponent(Sprite).spriteFrame=this.AutoSprites[2]
       let time=0
       let temptime=1000
          this.schedule(()=>{
            if(this.Stopautoplay){
                setTimeout(()=>{
                    ...
                    this.ballpool.Allbuttons[8].getComponent(Sprite).spriteFrame=this.AutoSprites[3]
                    this.ballpool.Allbuttons[8].node.getChildByName("Label").active=false
                    for(let i=1;i< this.ballpool.Allbuttons.length-1;i++){
            ...
            sys.localStorage.setItem("TotalAmount", this.ballpool.amount.getComponent(Label).string)
                ...
                if(check){
                    ...
                    switch(noofsel){
                        ...
                        case 2:
                            // console.log("inn");
                            console.log(this.ballpool.iamt,"increased amt");
                ...
                if(AutoPlayManager.Iflost[2]["enable"]==true){
                    console.log(this.ballpool.iamt,"inc amt");
              ...
              if(i== AutoPlayManager.Roundsnum){
                ...
                },AutoPlayManager.Roundsnum*1000+2000)
          }
        }
          },2.1,AutoPlayManager.Roundsnum-1,1)
        // }
      }
      ...
      if(this.inside==false){
        ...
        let s1=select[0]
          ...
          if(s2==1){
            let rand = randomRangeInt(0, this.ballpool.valuesArr.length)
        this.ballpool.resultpoints[0] = this.ballpool.valuesArr[rand];
          }
          else{
            if (s2 == 0) {
              ...
              let randno=randomRangeInt(0,this.ballpool.valuesArr.length)
              j++
              temparr.push(randno)
              while(j<s2){
                 randno=randomRangeInt(0,this.ballpool.valuesArr.length)
              ...
              for (let i = 0; i < temparr.length ; i++) {
                this.ballpool.resultpoints[i] = this.ballpool.valuesArr[temparr[i]];
              }
            }
          }
         
          console.log(this.ballpool.resultpoints ,"result arr");
          console.log(this.ballpool.valuesArr,"val arr");
        //   console.log( this.ballpool.correctcount,"cc");
        //   console.log(this.ballpool.resultpoints.length,"len");
            let k=0
            this.ballpool.resultpoints.sort((a,b)=>(a-b))
        ...
        this.ballpool.child[(this.ballpool.resultpoints[k])-1].getComponent(Sprite).spriteFrame = this.ballpool.wrongSprite
          for (let n = 0; n < this.ballpool.resultpoints.length; n++) {
            if (this.ballpool.resultpoints[k] == this.ballpool.valuesArr[n]) {
              this.ballpool.payoutchildren[this.ballpool.correctcount].getComponent(Sprite).spriteFrame = this.ballpool.payoutspritef
            ...
            //   this.ballpool.payoutchildren[this.ballpool.correctcount].getComponent(UITransform).height = 61
              ...
              this.ballpool.child[this.ballpool.resultpoints[k] - 1].getComponent(Sprite).spriteFrame = this.ballpool.resultsprite
              break;
            }
        }
        k++
      },0.1,this.ballpool.resultpoints.length-1)
          setTimeout(()=>{
            for(let n=0;n<this.ballpool.valuesArr.length;n++){
                if(this.ballpool.payoutchildren[n].getComponent(Sprite).spriteFrame==this.ballpool.payoutspritef){
